...
int main(int argc, char** argv) {
    ...
    std::string glbPath2 = "assets/sunjae.glb";
    ...
    GLuint whiteFallbackTexture = makeSolidTexture(255, 255, 255, 255);
    ...
    // Set up sampler-backed shader uniforms once so unit 0 is always valid.
    glUseProgram(meshProg);
    glUniform1i(glGetUniformLocation(meshProg,"uTexture"),0);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        for (auto& mesh:bodyGPU){
            ...
            glActiveTexture(GL_TEXTURE0);
            if (useTex) glBindTexture(GL_TEXTURE_2D,catTextures[mat.textureIndex]);
            else glBindTexture(GL_TEXTURE_2D,whiteFallbackTexture);
        ...
        if (hasSecondCat) {
            ...
            for (auto& mesh:body2GPU){
                ...
                glUniform1i(bodyUseTexLoc,useTex?1:0);
                glActiveTexture(GL_TEXTURE0);
                if (useTex) glBindTexture(GL_TEXTURE_2D,cat2Textures[mat.textureIndex]);
                else glBindTexture(GL_TEXTURE_2D,whiteFallbackTexture);
                glBindVertexArray(mesh.vao); glDrawArrays(GL_TRIANGLES,0,mesh.count);
            }
            // Second cat fur
